...
fig, ((ax2, cax2), (ax3, cax3)) = plt.subplots(figsize=(8,8/3*2), nrows=2, ncols=2,
...
# ax1.plot_date(mpltime, surfv, '-', label='3D Var')
# ax1.plot_date(mpltimeref, surfv_ref, '-', label='no DA')
...
pc = ax2.contourf(mpltime_2d, z, vv_ref, 15)
cb = fig.colorbar(pc, cax=cax2)
cb.set_label('primary production [mgC $\mathregular{m^{-3}}$ $\mathregular{d^{-1}}$]')
ax2.set_ylabel('depth [m]')
ax2.set_title('primary production without data assimilation (a)')
ax2.grid()
ax2.xaxis.axis_date()
...
difflim = 3
contours = np.linspace(-difflim, difflim, 31)
pc = ax3.contourf(mpltime_2d, z, var_diff, cmap='RdBu_r', levels=contours, extend='both')
cb = fig.colorbar(pc, cax=cax3)
cb.set_label('primary production difference [mgC $\mathregular{m^{-3}}$ $\mathregular{d^{-1}}$]')
ax3.set_ylabel('depth [m]')
ax3.set_title('impact of data assimilation on primary production (DA (3DVar) - no DA) (b)')
ax3.grid()
ax3.xaxis.axis_date()
...
plt.savefig(f'{OUTDIR}/hovdiff_{var}.png',dpi=600)
